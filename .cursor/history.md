...
**Why:** Let the user run a custom callback with the processed result (layout or key-mapped); block submit until validation passes; after submit, prevent further edits while keeping export/download.

**Affected files:** src/types/importer-state.ts, src/providers/state.ts, src/providers/types.ts, src/providers/useImporterStateSetters.ts, src/providers/useImporterActions.ts, src/providers/ImporterProvider.tsx, src/core/view/get-view-counts.ts, src/core/view/get-view-counts.test.ts, src/core/view/index.ts, src/hooks/useImporter.ts, src/hooks/useImporter.test.tsx, src/hooks/useSheetEditor.ts, src/hooks/useSheetEditor.test.tsx, docs/how-to.md, ai-context.md, .cursor/history.md.

### 2026-02-28 — Edit: removeRow and change log (changeLog, changeLogAsText)

**What changed:** **(1)** **removeRow(rowIndex)** added to the edit phase: removes the row at the given global index and re-indexes remaining rows (main thread; no Worker). **(2)** **Change log:** Provider state now includes **changeLog** (readonly array of **ChangeLogEntry**: `cell_edit` with rowIndex, cellKey, value, previousValue?, timestamp; `row_remove` with rowIndex, timestamp). **addChangeLogEntry** in context; **formatChangeLogAsText(entries)** in `src/types/change-log.ts` produces a human-readable string. **(3)** **useSheetEditor** and **useSheetView** expose **removeRow**, **changeLog**, **changeLogAsText**. Log entries are appended in parallel (queueMicrotask) so the log does not block the edit. **(4)** **processFile** clears **changeLog**. **(5)** Types: **ChangeLogEntry**, **ChangeLogEntryCellEdit**, **ChangeLogEntryRowRemove** and **formatChangeLogAsText** exported from the package. **(6)** **core/editor/immutable-update.ts**: **removeRow(sheet, rowIndex)**; tests for removeRow and formatChangeLogAsText; useSheetEditor test for removeRow + changeLog.

**Why:** Let users delete invalid or duplicate rows during the edit phase and keep a text log of what they changed (edits and row removals) for display or export.

**Affected files:** src/types/change-log.ts, src/types/change-log.test.ts, src/types/importer-state.ts, src/types/index.ts, src/core/editor/immutable-update.ts, src/core/editor/immutable-update.test.ts, src/core/editor/index.ts, src/providers/state.ts, src/providers/types.ts, src/providers/useImporterActions.ts, src/hooks/useSheetEditor.ts, src/hooks/useSheetEditor.test.tsx, src/core/view/hooks/useSheetView.ts, src/core/view/types/use-sheet-view-return.ts, src/index.ts, docs/how-to-edit.md, .cursor/docs/Architecture.md, ai-context.md, .cursor/history.md.

### 2026-02-28 — Result: convert sheet to user DTO for submit (toObjects, toObjectsWithKeyMap)

**What changed:** **(1)** **useSheetData()** now exposes **toObjects\<T\>(mapRow)** (map each row to a user-defined type; returns `T[]`) and **toObjectsWithKeyMap(keyMap)** (map sheet column keys to output attribute names; returns `Record<string, unknown>[]`). **(2)** Pure helpers in **core/view/export/sheet-to-objects.ts**: **getCellValue(row, key)**, **sheetRowsToObjects(rows, mapRow)**, **sheetToObjectsWithKeyMap(sheet, keyMap)**. **getCellValue** is exported from the package for use in custom mappers. **(3)** **docs/how-to-result.md** added; **docs/how-to.md**, **Architecture.md**, **ai-context.md** updated.

**Why:** Let the user convert the final Sheet to their desired object shape (e.g. `{ nombre, apellido }`) for API submit, or keep using the raw **sheet** without conversion.

**Affected files:** src/core/view/export/sheet-to-objects.ts, src/core/view/export/sheet-to-objects.test.ts, src/hooks/useSheetData.ts, src/hooks/useSheetData.test.tsx, src/core/view/index.ts, src/index.ts, docs/how-to-result.md, docs/how-to.md, .cursor/docs/Architecture.md, ai-context.md, .cursor/history.md.

### 2026-02-28 — Controllers: new cell transforms (string, number, date)

**What changed:** **(1)** New **string** cell transforms: **toLowerCase**, **slice** (params: start, end), **replace** (search, replacement), **replaceByRegex** (pattern, flags, replacement), **fillStart**, **fillEnd** (length, fill), **extractByRegex** (pattern, flags, group). **(2)** New **number** cell transforms: **numberAdd**, **numberMultiply**, **numberDivide**, **numberSubtract** (param: value), **numberRound** (mode: round|ceil|floor), **numberAbs**, **numberSqrt**, **numberLimit** (min, max), **numberPercent** (param: total). **(3)** New **date** cell transforms: **dateToOnlyTime**, **dateToOnlyDate**, **dateToTimeDate**, **dateToUtc**, **dateLimit** (min, max), **dateAdd**, **dateSubtract** (params: days, hours, minutes, seconds, ms, months, years). **(4)** All new transforms registered in **core/transform/worker/worker-registry.ts**; colocated tests added; **docs/transformers.md**, **docs/how-to-transformers.md**, **src/utils/controller/README.md**, **ai-context.md** updated.

**Why:** Provide a rich set of built-in cell transforms so layouts can reference them by id without custom code; same pattern as validators and sanitizers (Register(), params).

**Affected files:** src/utils/controller/string/_transform_.ts, src/utils/controller/number/_transform_.ts, src/utils/controller/date/_transform_.ts, src/core/transform/worker/worker-registry.ts, docs/transformers.md, docs/how-to-transformers.md, src/utils/controller/README.md, ai-context.md, .cursor/history.md.

### 2026-02-28 — Controllers: new cell validators (string, number, date, bool)

**What changed:** **(1)** New **string** cell validators: **string:byregex** (params: pattern, flags), **string:maxLength**, **string:minLength**, **string:email**, **string:phone**, **string:phoneInternational**, **string:phoneLocal**, **string:onlyNumbers**, **string:onlyLetters** (params: allowSpaces). **(2)** New **number** cell validators: **number:min**, **number:max**, **number:float**, **number:integer**, **number:nonNegative**, **number:nonPositive**, **number:nonZero**. **(3)** New **date** cell validators: **date:min**, **date:max**, **date:onlyYear**, **date:onlyTime**, **date:datetime**, **date:timestamp**, **date:utc**. **(4)** New **bool** context and validators: **bool:onlyTrue**, **bool:onlyFalse**. **(5)** All validators registered in **core/validator/worker/worker-registry.ts**; colocated tests added; **docs/validators.md**, **docs/how-to-validators.md**, **src/utils/controller/README.md**, **ai-context.md** updated.

**Why:** Provide a rich set of built-in cell validators so layouts can reference them by id without custom code; same pattern as sanitizers (Register(), params, error codes for I18n).

**Affected files:** src/utils/controller/string/_validator_.ts, src/utils/controller/number/_validator_.ts, src/utils/controller/date/_validator_.ts, src/utils/controller/bool/\*.ts, src/core/validator/worker/worker-registry.ts, docs/validators.md, docs/how-to-validators.md, src/utils/controller/README.md, ai-context.md, .cursor/history.md.

### 2026-02-28 — Controllers: new cell sanitizers (number, date, string, replace)

**What changed:** **(1)** New **number** context: cell sanitizers **number** (int, strip non-digits), **float**, **number:toStringId** (pad start), **number:toStringEnd** (pad end), with params **length** / **fill** where applicable. **(2)** New **date** context: **data** (formatted date+time), **data:year**, **data:data** (date only), **data:time**, **data:timestamp**. **(3)** **string** context extended: **string:minusculas**, **string:mayusculas**, **string:maxLength** (param **maxLength**), **string:trimAdd** / **string:trimPre** (params **length**, **fill**), **string:spaces** (collapse spaces), **nullToEmpty**. **(4)** New **replace** context: **replace-from-regex** (params **pattern**, **flags**, **replacement**), **replace-from-str** (params **search**, **replacement**). **(5)** All new sanitizers registered in **core/sanitizer/worker/worker-registry.ts**; colocated tests added; **docs/sanitizers.md**, **docs/how-to-sanitizer.md**, **src/utils/controller/README.md**, **ai-context.md** updated.

**Why:** Provide a rich set of built-in cell sanitizers for numeric, date, string normalization and replacement so layouts can reference them by id without custom code.

**Affected files:** src/utils/controller/number/_.ts, src/utils/controller/date/_.ts, src/utils/controller/string/_.ts (new sanitizers), src/utils/controller/replace/_.ts, src/core/sanitizer/worker/worker-registry.ts, docs/sanitizers.md, docs/how-to-sanitizer.md, src/utils/controller/README.md, ai-context.md, .cursor/history.md.
...
**Why:** Fulfil Construction Step 11 — telemetry (metrics per phase, isSlow, percentages, efficiency), virtualization API (getRows 1-based, README example), fuzzy header mapping (normalize + Levenshtein + greedy mapHeaders), persistence versioning and clear-after-submit recommendation.

**Affected files:** src/types/metrics.ts (new), metrics.test.ts (new), importer-state.ts, types/index.ts, providers (state, types, useImporterStateSetters, useImporterActions, ImporterProvider, usePersistSession), useImportSheet, useImporter, core/sanitizer/hooks/useSanitizerWorker, core/validator/hooks/useValidatorWorker, core/transform/hooks/useTransformWorker, core/view (useSheetView, types), core/view/types/persisted-state.ts, core/parser/utils/fuzzy-match.ts (new), fuzzy-match.test.ts (new), README.md, docs/how-to-view.md, ai-context.md, .cursor/docs/Architecture.md, .cursor/history.md, src/index.ts.
