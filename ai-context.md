...
1. User selects a file → call **`processFile(file)`** (from **`useImporter()`**). Parser runs automatically in a Worker (preview: first 10 rows); you get raw data and status updates.
2. Optionally call **`startFullImport()`** (from **`useImportSheet()`**) to parse the full file after the preview. **Note:** The preview happens automatically when you call `processFile()`; you only need `useImportSheet()` if you want to explicitly trigger full file parsing.
...
| **`useImportSheet()`**                                       | **Optional:** **`startFullImport()`** to parse the full file after preview. Preview happens automatically when you call `processFile()`; only use this hook if you need explicit full file parsing.                                                                                                                                                                                                                                                                                                                                                                                                                                                           |
