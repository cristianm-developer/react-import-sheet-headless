...
| **Sync vs async (cell/row sync, only table async) or table resilience**    | **"Design principles"** → **Data flow** (Sync Row Loop → Async Table Check). **"Utils"** → **Sync vs async** and **Resilience for async table**. **6. Validator**, **7. Transform**: pipeline phases, try/catch, EXTERNAL\_\* codes, AbortController for fetch.                                                                                                                                                                                                                                                                                                                                     |
...
- **Types:** Shared types live in **`src/types/`**; types used only by one context or module live in **`<module>/types/`** (e.g. `ImporterContext/types/`, `core/parser/types/`). **One type per file**; each file is re-exported from that folder’s `types/index.ts`. See _Folder structure → Types_.
...
- **Holds file and state:** `file`, `rawData`, `documentHash`, `status`, `result` (sheet), **`submitDone`** (true after the user’s submit callback has run). Optional **`onSubmit`** and **`submitKeyMap`** (Provider props) let the consumer receive the processed result via **`submit()`** (only when there are no validation errors); after submit, editing is disabled, export/download remain available. So when the user navigates between UI steps (e.g. "Carga" → "Mapeo" → "Validación"), file and progress are not lost; no prop drilling — any child uses hooks to read.
...
- **Progress via EventTarget:** Progress is **not** in Context. A single **EventTarget** dispatches **`importer-progress`** (and **`importer-aborted`** on cancel/unmount). Only the progress UI subscribes; the rest of the tree does not re-render. See _Progress and re-renders: EventTarget_ below.
- **Registry API:** Three `Registry` instances (validator, sanitizer, transform); exposes `registerValidator`, `registerSanitizer`, `registerTransform` (Zero-Bundle-Size). See **2. Setting** — _Registro agnóstico_.
- **Abort & lifecycle:** Exposes **`abort()`**; on abort or **unmount**, **terminates** active Workers (`worker.terminate()`), resets status, dispatches **`importer-aborted`**. See _Maturity & resilience → Process Abort & Worker Cleanup_.
...
| **`useSheetData()`**                  | Returns the **result** (final sheet), **errors**, **toObjects\<T\>(mapRow)** (convert rows to user DTO), and **toObjectsWithKeyMap(keyMap)** (sheet column key → output attribute name). No knowledge of Workers.                                                                                                                                                                                                                                                                                                                                                                                                                   |
| **`useSheetEditor()`**                | Exposes **`editCell`** (scoped validate + transform on a cell), **`removeRow(rowIndex)`** (remove a row; re-index remaining rows), **`canEdit`** (false after submit), and **`changeLog`** / **`changeLogAsText`** (log of edits and row removals, updated in parallel). When **submitDone** is true, **canEdit** is false and **editCell** / **removeRow** no-op; export/download remain available.                                                                                                                                                                                                                                |
...
const { editCell, removeRow, changeLog, changeLogAsText } = useSheetEditor();
```
...
- **Strict delta strategy:** The Worker must **not** return the full sheet after each step. It returns only **errors** and **deltas** (changed rows/cells). The main thread is a **viewer** that patches its local state; the Worker (or a single "pipeline" Worker) is the owner of the full in-memory truth during processing. See _Data efficiency: Delta strategy_ above.
...
- **Typed errors:** Export a **`SheetError`** (or equivalent) type so consumers can import it and TypeScript knows the shape: **`code`** (string), **`params`** (optional record), **`level`**, and optional **`message`** as fallback. See _Maturity & resilience → I18n of errors_: the UI translates `code` + `params` into localized text; the library stays language-agnostic.
...
- **Context- or module-specific types** (used only by one context or core module):
  ...
  - Examples:
    ...
    - **Edit (shared):** `src/types/edit.ts`, `src/types/paginated-result.ts`, `src/types/change-log.ts` — `EditCellParams`, `PaginatedResult`, `ChangeLogEntry`, `formatChangeLogAsText`.
...
| **Editor**    | `core/editor/`    | Expose result **sheet**, **editCell** (Worker), **removeRow** (main thread; re-index rows), and **changeLog** / **changeLogAsText** (log of edits and row removals, updated in parallel). Run scoped validation + transform on edit in a **Web Worker** (runEditPipeline); structural immutability for React.memo. |
...
**Registry Pattern:** Layouts reference functions by **name (string identifier)** only (e.g. `validators: ['required', 'email']`). The Provider maintains three `Registry` instances (validator, sanitizer, transform) that map names to functions. Workers import the same controller modules (ESM imports) and resolve by name from their own Registry. See **2. Setting** — _Registro agnóstico de validadores/sanitizers/transformadores_.
...
const isInListValidator = (value: unknown, row: RawSheetRow, params?: { list: unknown[] }) => {
  ...
  return params.list.includes(value)
    ? null
    : [{ code: 'NOT_IN_LIST', params: { value, list: params.list } }];
};
```
...
- **Abortability:** The same **AbortController** used for process cancellation (see _Process Abort & Worker Cleanup_) must be passed into the table-level runner so that any **fetch** to the backend can be cancelled when the user calls **`abort()`**. Otherwise a pending backend request could outlive the cancelled import.
...
- **Public API (consumer-facing):** All live in **`src/hooks/`** and are the **only** hooks re-exported from **`src/index.ts`** (see _Public API vs internal_). The library exposes **five specialized hooks** that consume the Provider context (see _Provider as brain, Hooks as interface_):
...
- **`src/index.ts`** — **single control point for the public API.** It exports **only** what the user needs:
  ...
  - It must **not** export: the Context instance, `useImporterContext`, internal worker hooks, or internal provider modules. This keeps the library surface clean and NPM-friendly (see _Public API vs internal_).
...
7. **Edit (6):** Provider exposes **result (sheet)**, **errors** (from sheet), **editCell**, **removeRow**, and **changeLog** / **changeLogAsText**. On edit, **runEditPipeline** runs in a **Web Worker** (edit.worker.ts): set cell value → cell/row/sheet validators → cell/row/sheet transforms (safe-first). **removeRow(rowIndex)** runs on the main thread (immutable remove + re-index). Each edit or remove appends a **ChangeLogEntry** (cell_edit or row_remove) to **changeLog**; **changeLogAsText** is a human-readable string (updated in parallel via microtask). **pageData** and **totalPages** are derived from sheet and page/pageSize; structural immutability for React.memo.
